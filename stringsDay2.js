...
function removeEekAndJoin(str) {
    return str.split('Eek!').join("").split("").reverse().join("")
}

// Part 3
// split on v and return first letter of each resulting string

function splitV(str) {
    let instruction = []
    let newArr = str.split('v')
    for (let j = 1; j < newArr.length; j++) {
        instruction.push(newArr[j].slice(0,1))
    }
